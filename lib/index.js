...
export const Config = z.object({
    ...
    anonymous: z.boolean().required(false)
        .description('True to skip the Authorization header (registration-free endpoints like the default OVHcloud one); defaults to true on the default free endpoint'),
 ...
 * throw one error listing every attempt. A non-anonymous attempt with no
 * resolvable API key is SKIPPED instead of failing — so a zero-key install
 * flows straight to the anonymous free fallback without a wasted request.
 */
async function transcribeWithFallback(ctx, resolved, fallbacks, ref, signal, cache) {
    ...
    throw new Error(`dsh-vision-proxy: all ${attempts.length} vision model(s) failed (${attempted} attempted).\n${errors.join('\n')}`);
}
        ...
        : resolved.apiKey !== ''
            ...
            : process.env.VISION_API_KEY !== undefined && process.env.VISION_API_KEY !== ''
                ...
                : process.env.DASHSCOPE_API_KEY !== undefined && process.env.DASHSCOPE_API_KEY !== ''
                    ? '$DASHSCOPE_API_KEY'
                    : 'UNSET (main model skipped; fallback chain applies)';
    const fallbackSummary = fallbacks.length === 0
        ? 'none'
        : fallbacks.map((f) => `${f.model}@${f.baseURL}${f.anonymous ? ' (anonymous)' : ''}`).join(', ');
    ctx.logger.info(`dsh-vision-proxy: route "${resolved.providerId}" wraps "${resolved.innerProvider}" · VLM model "${resolved.model}" @ ${resolved.baseURL} · apiKey from ${keySource} · timeout ${resolved.timeoutMs}ms · maxTokens ${resolved.maxTokens} · maxImagePixels ${resolved.maxImagePixels} · fallbacks: ${fallbackSummary} (key itself is never logged)`);
    ctx.logger.info(`dsh-vision-proxy: PRIVACY NOTICE — image bytes are sent over HTTPS to the configured VLM endpoint for transcription. Default without a key: anonymous free endpoint ${DEFAULT_FREE_BASE_URL} (${DEFAULT_FREE_MODEL}). Images leave your machine unless baseURL points at a local service (e.g. Ollama). For sensitive images, configure your own endpoint or uninstall this plugin.`);
...
export const _test = { transcribeWithFallback, transcribeImage, transcribeRequest, classifyHttpError, resolveApiKey, parseRetryAfter, hasImage, maybeDownscale };
