 ...
 * resolvable API key is SKIPPED instead of failing. Endpoints that just hit a
 * rate limit or timed out are cooled down for ENDPOINT_COOLDOWN_MS.
 */
...
export function apply(ctx, config) {
    ...
    ctx.logger.info(`dsh-vision-proxy: route "${resolved.providerId}" wraps "${resolved.innerProvider}" · VLM model "${resolved.model}" @ ${resolved.baseURL} · apiKey from ${keySource} · timeout ${resolved.timeoutMs}ms (anonymous capped at ${ANONYMOUS_TIMEOUT_CAP_MS}ms) · maxTokens ${resolved.maxTokens} · maxImagePixels ${resolved.maxImagePixels} · autoLocalOllama ${resolved.autoLocalOllama} · fallbacks: ${fallbackSummary} (key itself is never logged)`);
